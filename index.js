...
async function connectSession(sessionId) {
  ...
  if (sockets[sessionId]) {
    try { await sockets[sessionId].logout(); } catch {}
...
if (fs.existsSync(regexTriggerProPath)) {
  ...
  regexTriggersPro[sessionId] = [];
    ...
    if (connection === 'close') {
      ...
      if (
        code === DisconnectReason.StreamError ||
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    if (!text) return;
     ...
     // ✅ RegexTriggersPro (filter by target_number)
...
for (const trigger of proTriggers) {
  try {
    ...
    // ✅ Regex Triggers
 // ✅ Improved Regex Triggers
    const triggers = regexTriggers[sessionId] || [];
    const matchedTriggers = [];
    
    // First find all matching triggers
    for (const trigger of triggers) {
        try {
            const regex = new RegExp(trigger.regex, 'i'); // Add 'i' flag for case-insensitive
            if (regex.test(text)) {
    ...
    // Process matches (you can choose to handle all or just first)
    if (matchedTriggers.length > 0) {
        // Get the most specific match (longest regex pattern)
...
// 🔁 Reconnect (force reinitialize the session)
app.post('/api/v1/session/:sessionId/reconnect', verifyApiKey, async (req, res) => {
  const { sessionId } = req.params;

  try {
    await connectSession(sessionId);
    return res.json({ success: true, message: 'Session reconnected' });
  } catch (e) {
    return res.status(500).json({ error: e.toString() });
  }
});
